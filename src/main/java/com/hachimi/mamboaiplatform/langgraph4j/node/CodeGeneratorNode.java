...
public class CodeGeneratorNode {
  ...
  public static AsyncNodeAction<MessagesState<String>> create() {
    return node_async(state -> {
      ...
      if (context.getRetryCount() > 0) {
        log.info("执行节点: 代码生成 (第 {} 次重试)", context.getRetryCount());
      } else {
        log.info("执行节点: 代码生成");
      }
      ...
      // 根据类型设置生成目录
      String generatedCodeDir = String.format("%s/%s_%s", AppConstant.CODE_OUTPUT_ROOT_DIR, generationType.getValue(),
          appId);
  ...
  private static String buildUserMessage(WorkflowContext context) {
    ...
    if (isQualityCheckFailed(qualityResult)) {
      ...
      userMessage = buildErrorFixPrompt(qualityResult, context.getRetryCount());
    }
   ...
   * 构造错误修复提示词
   */
  private static String buildErrorFixPrompt(QualityResult qualityResult, Integer retryCount) {
    StringBuilder errorInfo = new StringBuilder();
    errorInfo.append(String.format("\n\n## 第 %d 次重试 - 上次生成的代码存在以下问题，请修复：\n", retryCount));
    ...
    errorInfo.append(String.format("\n请根据上述问题和建议重新生成代码，确保修复所有提到的问题。这是第 %d 次重试，请特别注意代码质量。", retryCount));
